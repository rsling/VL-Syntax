...
\begin{frame}
  ...
  \begin{exe}
    ...
    \begin{xlist}
      \ex{\label{ex:praedikative028} Er fischt \orongsch{[den Teich]} \alert{[leer]}.
      \onslide<5->{\alert{→ \orongsch{[Der Teich]} wird \alert{[leer]}.}}}
      \ex{\label{ex:praedikative029} Sie färbt \orongsch{[den Pullover]} \alert{[grün]}.
      \onslide<5->{\alert{→ \orongsch{[Der Pullover]} wird \alert{[grün]}.}}}
      \ex{\label{ex:praedikative030} Er stampft \orongsch{[die Äpfel]} \alert{[zu Brei]}.
      \onslide<5->{\alert{→ \orongsch{[Die Äpfel]} werden \alert{[zu Brei]}.}}}
    \end{xlist}
  \end{exe}
  \pause
  \begin{itemize}[<+->]
    \item Kann man ergänzen: \textit{\orongsch{(Objekt-NP)} ist\slash wird \alert{(Resultativprädikat)}}.
      \Halbzeile

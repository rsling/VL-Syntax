...
\begin{quote}
  ...
  \begin{spread}
    ...
    Die Gründe, die dafür zu sprechen scheinen, sind folgende: a = a und a = b sind offenbar Sätze von verschiedenem Erkenntniswerte: a = a gilt a priori und ist nach Kant analytisch zu nennen, während \Solalt{[Sätze von der Form a = b oft sehr wertvolle Erweiterungen unserer Erkenntnis enthalten und a priori nicht immer zu begründen sind]}{Sätze von der Form a = b oft sehr wertvolle Erweiterungen unserer Erkenntnis enthalten und a priori nicht immer zu begründen sind}.
    Die Entdeckung, daß \Solalt{[nicht jeden Morgen eine neue Sonne aufgeht, sondern immer dieselbe,]}{nicht jeden Morgen eine neue Sonne aufgeht, sondern immer dieselbe,} ist wohl eine der folgenreichsten in der Astronomie gewesen.
    Noch jetzt ist die Wiedererkennung eines kleinen Planeten oder eines Kometen nicht immer etwas Selbstverständliches.
    Wenn \Solalt{[wir nun in der Gleichheit eine Beziehung zwischen dem sehn wollten, was die Namen »a« und »b« bedeuten,]}{wir nun in der Gleichheit eine Beziehung zwischen dem sehn wollten, was die Namen »a« und »b« bedeuten,} so schiene a = b von a = a nicht verschieden sein zu können, falls \Solalt{[nämlich a = b wahr ist]}{nämlich a = b wahr ist}.
...
\Sol{%
  \begin{itemize}
    \item[]% 
      \begin{tikzpicture}[node distance=1cm, auto,]
        \node[] (context) {(2)~(ob Penny)};
        \node[right=of context] (schweigen) {schweigen};
        \node[right=of schweigen] (kann) {kann};
        \path[->, draw, bend left=45] (kann) edge node[below] {1} (schweigen);
      \end{tikzpicture}
    \item[]% 
      \begin{tikzpicture}[node distance=1cm, auto,]
        \node[] (context) {(3)~(obwohl Juliette)};
        \node[right=of context] (baden) {baden};
        \node[right=of baden] (können) {können};
        \node[right=of können] (wollte) {wollte};
        \path[->, draw, bend left=45] (wollte) edge node[below] {1} (können);
        \path[->, draw, bend left=45] (können) edge node[below] {1} (baden);
      \end{tikzpicture}
    \item[]% 
      \begin{tikzpicture}[node distance=1cm, auto,]
        \node[] (context) {(4)~(weil Michelle)};
        \node[right=of context] (gestresst) {gestresst};
        \node[right=of gestresst] (gewesen) {gewesen};
        \node[right=of gewesen] (sein) {sein};
        \node[right=of sein] (wird) {wird};
        \path[->, draw, bend left=45] (wird) edge node[below] {1} (sein);
        \path[->, draw, bend left=45] (sein) edge node[below] {3} (gewesen);
        \path[->, draw, bend left=45] (gewesen) edge node[below] {3} (gestresst);
      \end{tikzpicture}
  \end{itemize}
}
...
\Sol{\textbf{Hinweis: Fragen Sie \uline{auf keinen Fall} nach einer Lösung für diese Aufgabe. Es handelt sich wie gesagt um eine \uline{Entlastungsaufgabe}. Wenn Sie nicht beizeiten darüber nachgedacht haben, ist die Aufgabe wertlos für Sie.}}
